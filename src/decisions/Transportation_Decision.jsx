          ...
          <Thead className="bg-gray-100 text-gray-700">
            <Tr>
              ...
              {Object.keys(data.product0.surface).map((carrier) => (
                <Th key={carrier} className="py-2 px-4" textAlign="center">Carrier <span className="text-red-500">{carrier}</span></Th>
              ))}
            </Tr>
          </Thead>
          <Tbody>
              ...
              <div className=" pl-4 flex space-x-4">
                {Array.from(
                  ...
                  (_, i) => (
                    <div
                      key={i + 1}
                      onClick={() => setSelectedQuarter(i + 1)} // This correctly updates the selected quarter
            ...
            <div className="m-2 min-w-[700px] rounded-2xl bg-white p-2 flex flex-col space-y-4 justify-start">
              ...
              {!(TransportationData?.flag_dc3 || TransportationData?.flag_dc2) && (
                <div style={{ padding: "20px", backgroundColor: "#f9f9f9", borderRadius: "8px", border: "1px solid #e0e0e0" }}>
                  <h2 style={{ color: "#333", fontSize: "24px", fontWeight: "bold", marginBottom: "12px" }}>No Distribution Center Available</h2>
                  <p style={{ color: "#555", fontSize: "16px", lineHeight: "1.6" }}>
                    Since your firm does not own a distribution center in this market region, all orders will be serviced from the distribution center associated with your manufacturing plant in Market Region 1.
                  </p>
                  <p style={{ color: "#555", fontSize: "16px", lineHeight: "1.6" }}>
                    To ensure prompt delivery within the current month, your firm will ship via air, which incurs higher transportation costs.
                  </p>
                  <p style={{ color: "#555", fontSize: "16px", lineHeight: "1.6" }}>
                    Please note that shipping costs are significantly higher when servicing from a distant DC, especially for direct-channel orders (Channel 2), which involve smaller quantities and therefore higher per-unit costs.
                  </p>
                  <p style={{ color: "#555", fontSize: "16px", lineHeight: "1.6" }}>
                    Consider establishing a local DC in this region in the future to reduce costs and improve service levels.
                  </p>
                </div>
              )}
            </div>
