...
const Manufacturing_Decisions = () => {
  ...
  const [PreviousManufacturingData, setPreviousManufacturingData] = useState({});
  const [lastValidManufacturingData, setLastValidManufacturingData] =
    useState(null); // Store last valid data
  ...
  const firm_data = selectedSim[0]?.firm_data
    ? Object.keys(selectedSim[0].firm_data)[0]
    : null;
  ...
  const getPreviousManufacturing = async () => {
    try {
      const response = await axios.get(`${api}/previous/`, {
        params: {
          user_id: user.userid,
          ...
          admin_id: selectedSim[0].admin_id,
          current_decision: "Manufacture",
          current_quarter: selectedQuarter-1,
          firm_key: firm_key_new,
        },
      });
      const data = response.data;
      setPreviousManufacturingData(data);
    } catch (error) {
      console.error("Error making GET request:", error);
    }
  };
  ...
  const handleChange = (channel, region, newValue) => {
    setValues((prevValues) => {
      ...
      // Validation for VolumeFlexibility and EmergencyLimit
      if (channel === "VolumeFlexibility") {
        if (parsedValue > 12500) {
          parsedValue = 12500;
        }
      } else if (channel === "EmergencyLimit") {
        ...
        const baseCapZero = 
          10000 + 
          (PreviousManufacturingData.production_zero || 0) +
          (PreviousManufacturingData.volume_flexibility_zero || 0);
  
        const baseCapHyperware = 
          10000 + 
          (PreviousManufacturingData.production_hyperware || 0) +
          (PreviousManufacturingData.volume_flexibility_hyperware || 0);
  
        const baseCapMetaware = 
          10000 + 
          (PreviousManufacturingData.production_metaware || 0) +
          (PreviousManufacturingData.volume_flexibility_metaware || 0);
  
        // Apply cap to the corresponding production region
        if (region === "productZero" && parsedValue > baseCapZero) {
          parsedValue = baseCapZero;
        } else if (region === "hyperware" && parsedValue > baseCapHyperware) {
          parsedValue = baseCapHyperware;
        } else if (region === "metaware" && parsedValue > baseCapMetaware) {
          parsedValue = baseCapMetaware;
        }
      }
        ...
        <div className="sm:grid grid-cols-1 gap-3 m-1 ">
          <div className="m-3 rounded-2xl bg-white p-2 flex flex-col justify-start custom-shadow">
            ...
            {loading ? (
              ...
              <>
                <Table
                  id="manufacture-production-table"
                  className="w-30 bg-white rounded-md shadow-sm"
                >
                      ...
                      <Tr key={channel} className="border-t">
                        ...
                        <Td className="p-3">
                          <Input
                            ...
                            disabled={channel === "EmergencyLimit"} // Disable for EmergencyLimit
            ...
            <div className="flex justify-end mt-4">
              ...
              >
                {loading ? <Spinner size="sm" /> : "Submit Manufacture"}{" "}
