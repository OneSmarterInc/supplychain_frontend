...
import {
  ForecastingSales2View,
  ForecastingSalesView,
} from "./view/ForecastView";
...
import DistributionView from "./view/DistributionView";
...
const BackOfficeDecision = () => {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      ...
      try {
        setLoading(true);
        const response = await axios.get(
          `${api}/get-firms/${selectedCourse.passcode}/`
        );
  ...
  const handleDecisionClick = async (decision) => {
    if (
      !selectedQuarter ||
      !selectedCourse?.simulationId ||
      !selectedTeam?.teamName
    )
      return;

    const queryParams = new URLSearchParams({
      sim_id: selectedCourse.simulationId,
      admin_id: 1,
    ...
    try {
      ...
      toast.error("Decision not yet submitted for this quarter.");
          ...
          <div className="px-4 lg:px-10 mt-10 rounded-2xl bg-white py-6 flex flex-col justify-start mx-4">
              ...
              <div className="flex flex-col lg:flex-row items-center lg:space-x-4 mb-6">
                ...
                <div className="mt-2 lg:mt-0 w-full lg:w-auto flex flex-wrap gap-2">
                  {teams.map((team, index) => (
                    ...
                    >
                      {team.teamName} <span className="px-2">|</span>{" "}
                      {team.users.length || 0}{" "}
                      <i className="fa fa-user text-sm pl-1"></i>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col lg:flex-row items-center lg:space-x-4 mb-6 pt-4">
              ...
              <div className="mt-2 lg:mt-0 flex space-x-2 lg:space-x-4">
                {Array.from(
                  { length: selectedCourse?.currentQuarter || 0 },
                  (_, i) => (
                    <div
                      key={i + 1}
                      onClick={() => handleQuarterClick(i + 1)}
                      className={`flex items-center justify-center w-8 h-8 rounded-full border border-gray-300 text-gray-700 cursor-pointer ${
                        selectedQuarter === i + 1
            ...
            {loading ? (
              ...
              <>
                ...
                {selectedDecision === "Distribution" && (
                  <div className="overflow-auto mt-10">
                    {/* Render procurement-related tables when 'Procurement' decision is selected */}
                    <DistributionView data={selectedCourse} repo={reportData} />
                  </div>
                )}
                ...
                {selectedDecision === "Demand" && (
                  <div className="overflow-auto mt-10">
                    {/* Render procurement-related tables when 'Procurement' decision is selected */}
                    <DemandView
                      data={selectedCourse.renamedMappedData}
                      repo={reportData}
                    />
                  </div>
                )}
